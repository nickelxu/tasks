...
DATASET_ID = "AICost"
TABLE_NAME = "gcp_billing_export_v1_01A413_A6C40C_96C18F"  # 使用标准导出表
...
def get_billing_data():
    try:
        ...
        # 3. 按 Project 细分 (因为 API Key 通常不在标准导出中，先按项目分)
        # 如果需要按 Label 分，可以修改这里
        query_by_project = f"""
            SELECT
        ...
        project_costs = list(client.query(query_by_project).result())
        ...
        return {
            ...
            "projects": [{"id": row.project_id, "name": row.project_name, "cost": row.cost} for row in project_costs],
...
def send_feishu_card(data):
    ...
    for p in data['projects']:
        if p['cost'] > 0: # 只显示有消耗的项目
            project_text += f"• {p['name'] or p['id']}: ${p['cost']:.2f}\n"
    ...
    for t in data['trend']:
        icon = "⚪"
        if t['cost'] > 100: icon = "🔴"
        elif t['cost'] > 50: icon = "jq" # 假设的颜色代码，实际用圆点
            ...
            },
            {
            ...
            },
             {
                ...
                "elements": [
                    {
                        "tag": "plain_text",
                        "content": f"📦 关联项目数量: {len(data['projects'])}"
                    }
                ]
            }
        ]
